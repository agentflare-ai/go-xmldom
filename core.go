...
func (n *node) BaseURI() DOMString {
	...
	if n.nodeType == DOCUMENT_NODE {
		if doc, ok := n.ownerDocument.(*document); ok {
			if doc.url != "" {
				return doc.url
			}
			if doc.documentURI != "" {
...
func (d *document) CloneNode(deep bool) Node {
	// Clone the underlying node tree first
	nodeClone, ok := d.node.CloneNode(deep).(*node)
	if !ok {
		return nil
	}
	cloned := &document{
		...
		// idMap is intentionally not copied to avoid referencing elements from the source document
		idMap:        nil,
		url:          d.url,
	...
	// Fix ownerDocument recursively for the cloned tree to point at the new document
	var setOwner func(Node)
	setOwner = func(n Node) {
		if internal := getInternalNode(n); internal != nil {
			internal.ownerDocument = cloned
		}
		...
		if attrs := n.Attributes(); attrs != nil {
			for i := uint(0); i < attrs.Length(); i++ {
				setOwner(attrs.Item(i))
			}
		}
	}
	setOwner(cloned)

	// Determine and set the document element based on the cloned children
	if deep {
		for child := cloned.FirstChild(); child != nil; child = child.NextSibling() {
			if child.NodeType() == ELEMENT_NODE {
				if elem, ok := child.(Element); ok {
					cloned.documentElement = elem
				}
				break
			}
		}
...
func (a *attr) CloneNode(deep bool) Node {
	// Clone as an attribute node, preserving name/value and namespace fields.
	// ownerElement is intentionally not copied per DOM spec semantics.
	return &attr{
		node: node{
			nodeType:      ATTRIBUTE_NODE,
			nodeName:      a.nodeName,
			...
			// child/parent/sibling pointers are nil for attributes
		},
		ownerElement: nil,
	}
}
...
func (cd *cdataSection) CloneNode(deep bool) Node {
	return &cdataSection{
		text: text{
			characterData: characterData{
				node: node{
					nodeType:      CDATA_SECTION_NODE,
					nodeName:      "#cdata-section",
					nodeValue:     cd.nodeValue,
					ownerDocument: cd.ownerDocument,
				},
			},
		},
	}
...
func (pi *processingInstruction) CloneNode(deep bool) Node {
	return &processingInstruction{
		node: node{
			nodeType:      PROCESSING_INSTRUCTION_NODE,
			nodeName:      pi.nodeName,  // typically the target
			nodeValue:     pi.nodeValue, // typically the data
